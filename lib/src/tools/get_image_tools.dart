...
/*
  static Future<void> fetchImageAndGeneratePalette(
      Function(PaletteGenerator?) onPaletteGenerated, String imagePath) async {
    final receivePort = ReceivePort();
    final isolate =
        await Isolate.spawn(_getImageColorIsolate, receivePort.sendPort);
    final sendPort = await receivePort.first;
    final result = ReceivePort();
    ...
    try {
      final paletteGenerator = await result.first;
      onPaletteGenerated(paletteGenerator);
    } finally {
      result.close();
      receivePort.close();
      isolate.kill();
  ...
  static void _getImageColorIsolate(SendPort mainSendPort) async {
    final receivePort = ReceivePort();
    ...
    await for (final message in receivePort) {
      ...
      try {
        ...
        if (image != null) {
          final paletteGenerator = await PaletteGenerator.fromImageProvider(
            MemoryImage(rawImage),
          );
          responseSendPort.send(paletteGenerator);
        } else {
          debugPrint('Error: Failed to decode image');
        }
      } catch (e) {
